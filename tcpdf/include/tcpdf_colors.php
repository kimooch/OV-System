...
class TCPDF_COLORS
{
    ...
    public static function convertHTMLColorToDec(
        ...
        if (strpos($color, "[") !== false) {
            if (
                ...
                switch ($m[1]) {
                    case "cmyk":
                        ...
                        $returncolor["C"] = max(
                            0,
                            min(100, floatval($m[2]) * 100)
                        );
                        $returncolor["M"] = max(
                            0,
                            min(100, floatval($m[3]) * 100)
                        );
                        $returncolor["Y"] = max(
                            0,
                            min(100, floatval($m[4]) * 100)
                        );
                        $returncolor["K"] = max(
                            0,
                            min(100, floatval($m[5]) * 100)
                        );
                        break;
                    case "rgb":
                        // RGB
                        $returncolor["R"] = max(
                            0,
                            min(255, floatval($m[2]) * 255)
                        );
                        $returncolor["G"] = max(
                            0,
                            min(255, floatval($m[3]) * 255)
                        );
                        $returncolor["B"] = max(
                            0,
                            min(255, floatval($m[4]) * 255)
                        );
                        break;
                    case "g":
                        // grayscale
            ...
            substr($color, 0, 4) != "cmyk" and
            substr($color, 0, 3) != "rgb" and
            ($dotpos = strpos($color, ".")) !== false
        ) {
        ...
        switch (strlen($color_code)) {
            ...
            case 6:
                ...
                $returncolor["R"] = max(
                    0,
                    min(255, hexdec(substr($color_code, 0, 2)))
                );
                $returncolor["G"] = max(
                    0,
                    min(255, hexdec(substr($color_code, 2, 2)))
                );
                $returncolor["B"] = max(
                    0,
                    min(255, hexdec(substr($color_code, 4, 2)))
                );
                break;
            case 8:
                ...
                $returncolor["C"] = max(
                    0,
                    min(100, round(hexdec(substr($color_code, 0, 2)) / 2.55))
                );
                $returncolor["M"] = max(
                    0,
                    min(100, round(hexdec(substr($color_code, 2, 2)) / 2.55))
                );
                $returncolor["Y"] = max(
                    0,
                    min(100, round(hexdec(substr($color_code, 4, 2)) / 2.55))
                );
                $returncolor["K"] = max(
                    0,
                    min(100, round(hexdec(substr($color_code, 6, 2)) / 2.55))
                );
                break;
            default:
                $returncolor = $defcol;
    ...
    public static function getColorStringFromArray($c)
    {
        ...
        switch (count($c)) {
            case 4:
                ...
                $color .= sprintf(
                    "%F %F %F %F",
                    max(0, min(100, floatval($c[0]))) / 100,
            ...
            case 3:
                ...
                $color .= sprintf(
                    "%F %F %F",
                    max(0, min(255, floatval($c[0]))) / 255,
