...
class TCPDF_FILTERS
{
     ...
     * @private static
     */
    private static $available_filters = [
        "ASCIIHexDecode",
        "ASCII85Decode",
        "LZWDecode",
        "FlateDecode",
    ...
    public static function decodeFilterASCII85Decode($data)
    {
        ...
        switch ($group_pos) {
            case 4:
                $decoded .=
                    chr($tuple >> 24) . chr($tuple >> 16) . chr($tuple >> 8);
                break;
            case 3:
    ...
    public static function decodeFilterLZWDecode($data)
    {
        ...
        while (
            $data_length > 0 and
            ($index = bindec(substr($bitstring, 0, $bitlen))) != 257
        ) {
            ...
            if ($index == 256) {
                ...
                if ($index < $dix) {
                    ...
                    $decoded .= $dictionary[$index];
                    $dic_val =
                        $dictionary[$prev_index] . $dictionary[$index][0];
                    // store current index
                    $prev_index = $index;
                } else {
                    // index do not exist on dictionary
                    $dic_val =
                        $dictionary[$prev_index] . $dictionary[$prev_index][0];
                    $decoded .= $dic_val;
                }
                // update dictionary
