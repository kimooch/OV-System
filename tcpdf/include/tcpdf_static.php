...
class TCPDF_STATIC
{
     ...
     * @public static
     */
    public static function getBorderMode(
        $brd,
        $position = "start",
        $opencell = true
    ) {
    ...
    public static function getObjFilename($type = "tmp", $file_id = "")
    {
        return tempnam(
            ...
            "__tcpdf_" .
                ...
                $type .
                "_" .
                md5(TCPDF_STATIC::getRandomSeed()) .
                "_"
        );
    }
    ...
    {
        if (
            $object instanceof Imagick and
            version_compare(phpversion("imagick"), "3.0.1") !== 1
        ) {
    ...
    {
        if (
            !isset($_SERVER["HTTP_ACCEPT_ENCODING"]) or
    ...
    {
        if (
            function_exists("mcrypt_encrypt") and
            ($out = @mcrypt_encrypt(
                MCRYPT_ARCFOUR,
                $key,
                $text,
     ...
     * @public static
     */
    public static function getAnnotOptFromJSProp(
        $prop,
        &$spot_colors,
        $rtl = false
    ) {
        ...
        if (
            isset($prop["buttonFitBounds"]) and
            $prop["buttonFitBounds"] == "true"
        ) {
            $opt["mk"]["if"]["fb"] = true;
        }
        ...
        if (isset($prop["fillColor"])) {
            if (is_array($prop["fillColor"])) {
                ...
                $opt["mk"]["bg"] = TCPDF_COLORS::convertHTMLColorToDec(
                    $prop["fillColor"],
                    $spot_colors
                );
            }
        ...
        if (isset($prop["strokeColor"])) {
            if (is_array($prop["strokeColor"])) {
                ...
                $opt["mk"]["bc"] = TCPDF_COLORS::convertHTMLColorToDec(
                    $prop["strokeColor"],
                    $spot_colors
                );
            }
        ...
        if (
            isset($prop["multipleSelection"]) and
        ...
        if (
            isset($prop["doNotSpellCheck"]) and
            $prop["doNotSpellCheck"] == "true"
        ) {
            $ff += 1 << 22;
        }
        ...
        if (
            isset($prop["radiosInUnison"]) and
            $prop["radiosInUnison"] == "true"
        ) {
            $ff += 1 << 25;
        }
        ...
        if (
            isset($prop["commitOnSelChange"]) and
            $prop["commitOnSelChange"] == "true"
        ) {
            $ff += 1 << 26;
        }
        $opt["ff"] = $ff;
        ...
        if (
            isset($prop["currentValueIndices"]) and
            is_array($prop["currentValueIndices"])
        ) {
    ...
    public static function extractCSSproperties($cssdata)
    {
        ...
        foreach ($cssblocks as $block) {
            ...
            $c += intval(
                preg_match_all(
                    "/[\:]link|visited|hover|active|focus|target|lang|enabled|disabled|checked|indeterminate|root|nth|first|last|only|empty|contains|not/i",
                    $selector,
                    $matches
                )
     ...
     * @public static
     */
    public static function fixHTMLCode(
        $html,
        $default_css,
        $tagvs,
        $tidy_options,
    ...
    public static function isValidCSSSelectorForTag($dom, $key, $selector)
    {
        ...
        $class = [];
        if (
            isset($dom[$key]["attribute"]["class"]) and
        ...
        $id = "";
        if (
            isset($dom[$key]["attribute"]["id"]) and
            ...
            if ($lasttag == "*" or $lasttag == $tag) {
                ...
                if ($valid and $offset > 0) {
                    ...
                    switch ($operator) {
                        ...
                        case ">":
                            // child of an element
                            $valid = self::isValidCSSSelectorForTag(
                                $dom,
                                $dom[$key]["parent"],
                                $selector
                            );
        ...
        if (
            ...
            foreach ($transform as $key => $data) {
                if (!empty($data[2])) {
                    ...
                    $tm = self::getTransformationMatrixProduct($tm, [
                        $a,
                        $b,
                        $c,
                        $d,
    ...
    public static function pregSplit(
        ...
        if (strlen($subject) > 0) {
            $ret = array_merge(
                $ret,
                preg_split($pattern . $modifiers, $subject, $limit, $flags)
            );
        }
        return $ret;
    }
    ...
    public static function fileGetContents($file)
    {
        ...
        if (
            ...
            if ($findroot === false || $findroot > 1) {
                $alt[] = htmlspecialchars_decode(
                    urldecode($_SERVER["DOCUMENT_ROOT"] . $file)
                );
            }
        }
        //
    ...
    public static $page_formats = [
        ...
        "A1" => [1683.78, 2383.937], // = (  594 x 841  ) mm  = ( 23.39 x 33.11 ) in
        "A2" => [1190.551, 1683.78], // = (  420 x 594  ) mm  = ( 16.54 x 23.39 ) in
        "A3" => [841.89, 1190.551], // = (  297 x 420  ) mm  = ( 11.69 x 16.54 ) in
        "A4" => [595.276, 841.89], // = (  210 x 297  ) mm  = (  8.27 x 11.69 ) in
        ...
        "A12" => [36.85, 51.024], // = (   13 x 18   ) mm  = (  0.51 x 0.71  ) in
        ...
        "B3" => [1000.63, 1417.323], // = (  353 x 500  ) mm  = ( 13.90 x 19.69 ) in
        "B4" => [708.661, 1000.63], // = (  250 x 353  ) mm  = (  9.84 x 13.90 ) in
        ...
        "B12" => [42.52, 62.362], // = (   15 x 22   ) mm  = (  0.59 x 0.87  ) in
        // ISO 216 C Series + 2 SIS 014711 extensions + 5 EXTENSION
        "C0" => [2599.37, 3676.535], // = (  917 x 1297 ) mm  = ( 36.10 x 51.06 ) in
        "C1" => [1836.85, 2599.37], // = (  648 x 917  ) mm  = ( 25.51 x 36.10 ) in
        "C2" => [1298.268, 1836.85], // = (  458 x 648  ) mm  = ( 18.03 x 25.51 ) in
        ...
        "C6" => [323.15, 459.213], // = (  114 x 162  ) mm  = (  4.49 x 6.38  ) in
        "C7" => [229.606, 323.15], // = (   81 x 114  ) mm  = (  3.19 x 4.49  ) in
        "C8" => [161.575, 229.606], // = (   57 x 81   ) mm  = (  2.24 x 3.19  ) in
        "C9" => [113.386, 161.575], // = (   40 x 57   ) mm  = (  1.57 x 2.24  ) in
        "C10" => [79.37, 113.386], // = (   28 x 40   ) mm  = (  1.10 x 1.57  ) in
        "C11" => [56.693, 79.37], // = (   20 x 28   ) mm  = (  0.79 x 1.10  ) in
        ...
        "DLE" => [323.15, 637.795], // = (  114 x 225  ) mm  = (  4.49 x 8.86  ) in
        "DLX" => [340.158, 666.142], // = (  120 x 235  ) mm  = (  4.72 x 9.25  ) in
        "DLP" => [280.63, 595.276], // = (   99 x 210  ) mm  = (  3.90 x 8.27  ) in (1/3 A4)
        // SIS 014711 E Series
        "E0" => [2491.654, 3517.795], // = (  879 x 1241 ) mm  = ( 34.61 x 48.86 ) in
        "E1" => [1757.48, 2491.654], // = (  620 x 879  ) mm  = ( 24.41 x 34.61 ) in
        "E2" => [1247.244, 1757.48], // = (  440 x 620  ) mm  = ( 17.32 x 24.41 ) in
        "E3" => [878.74, 1247.244], // = (  310 x 440  ) mm  = ( 12.20 x 17.32 ) in
        "E4" => [623.622, 878.74], // = (  220 x 310  ) mm  = (  8.66 x 12.20 ) in
        "E5" => [439.37, 623.622], // = (  155 x 220  ) mm  = (  6.10 x 8.66  ) in
        "E6" => [311.811, 439.37], // = (  110 x 155  ) mm  = (  4.33 x 6.10  ) in
        ...
        "E12" => [36.85, 53.858], // = (   13 x 19   ) mm  = (  0.51 x 0.75  ) in
        // SIS 014711 G Series
        "G0" => [2715.591, 3838.11], // = (  958 x 1354 ) mm  = ( 37.72 x 53.31 ) in
        "G1" => [1919.055, 2715.591], // = (  677 x 958  ) mm  = ( 26.65 x 37.72 ) in
        "G2" => [1357.795, 1919.055], // = (  479 x 677  ) mm  = ( 18.86 x 26.65 ) in
        "G3" => [958.11, 1357.795], // = (  338 x 479  ) mm  = ( 13.31 x 18.86 ) in
        "G4" => [677.48, 958.11], // = (  239 x 338  ) mm  = (  9.41 x 13.31 ) in
        "G5" => [479.055, 677.48], // = (  169 x 239  ) mm  = (  6.65 x 9.41  ) in
        "G6" => [337.323, 479.055], // = (  119 x 169  ) mm  = (  4.69 x 6.65  ) in
        "G7" => [238.11, 337.323], // = (   84 x 119  ) mm  = (  3.31 x 4.69  ) in
        "G8" => [167.244, 238.11], // = (   59 x 84   ) mm  = (  2.32 x 3.31  ) in
        ...
        "A3_SUPER" => [864.567, 1440.0], // = (  305 x 508  ) mm  = ( 12.01 x 20.00 ) in
        ...
        // ANSI Series
        "ANSI_E" => [2448.0, 3168.0], // = (  864 x 1118 ) mm  = ( 34.00 x 44.00 ) in
        "ANSI_D" => [1584.0, 2448.0], // = (  559 x 864  ) mm  = ( 22.00 x 34.00 ) in
        "ANSI_C" => [1224.0, 1584.0], // = (  432 x 559  ) mm  = ( 17.00 x 22.00 ) in
        "ANSI_B" => [792.0, 1224.0], // = (  279 x 432  ) mm  = ( 11.00 x 17.00 ) in
        "ANSI_A" => [612.0, 792.0], // = (  216 x 279  ) mm  = (  8.50 x 11.00 ) in
        // Traditional 'Loose' North American Paper Sizes
        "USLEDGER" => [1224.0, 792.0], // = (  432 x 279  ) mm  = ( 17.00 x 11.00 ) in
        "LEDGER" => [1224.0, 792.0], // = (  432 x 279  ) mm  = ( 17.00 x 11.00 ) in
        "ORGANIZERK" => [792.0, 1224.0], // = (  279 x 432  ) mm  = ( 11.00 x 17.00 ) in
        "BIBLE" => [792.0, 1224.0], // = (  279 x 432  ) mm  = ( 11.00 x 17.00 ) in
        "USTABLOID" => [792.0, 1224.0], // = (  279 x 432  ) mm  = ( 11.00 x 17.00 ) in
        "TABLOID" => [792.0, 1224.0], // = (  279 x 432  ) mm  = ( 11.00 x 17.00 ) in
        "ORGANIZERM" => [612.0, 792.0], // = (  216 x 279  ) mm  = (  8.50 x 11.00 ) in
        "USLETTER" => [612.0, 792.0], // = (  216 x 279  ) mm  = (  8.50 x 11.00 ) in
        "LETTER" => [612.0, 792.0], // = (  216 x 279  ) mm  = (  8.50 x 11.00 ) in
        "USLEGAL" => [612.0, 1008.0], // = (  216 x 356  ) mm  = (  8.50 x 14.00 ) in
        "LEGAL" => [612.0, 1008.0], // = (  216 x 356  ) mm  = (  8.50 x 14.00 ) in
        "GOVERNMENTLETTER" => [576.0, 756.0], // = (  203 x 267  ) mm  = (  8.00 x 10.50 ) in
        "GLETTER" => [576.0, 756.0], // = (  203 x 267  ) mm  = (  8.00 x 10.50 ) in
        "JUNIORLEGAL" => [576.0, 360.0], // = (  203 x 127  ) mm  = (  8.00 x 5.00  ) in
        "JLEGAL" => [576.0, 360.0], // = (  203 x 127  ) mm  = (  8.00 x 5.00  ) in
        // Other North American Paper Sizes
        "QUADDEMY" => [2520.0, 3240.0], // = (  889 x 1143 ) mm  = ( 35.00 x 45.00 ) in
        "SUPER_B" => [936.0, 1368.0], // = (  330 x 483  ) mm  = ( 13.00 x 19.00 ) in
        "QUARTO" => [648.0, 792.0], // = (  229 x 279  ) mm  = (  9.00 x 11.00 ) in
        "GOVERNMENTLEGAL" => [612.0, 936.0], // = (  216 x 330  ) mm  = (  8.50 x 13.00 ) in
        "FOLIO" => [612.0, 936.0], // = (  216 x 330  ) mm  = (  8.50 x 13.00 ) in
        "MONARCH" => [522.0, 756.0], // = (  184 x 267  ) mm  = (  7.25 x 10.50 ) in
        "EXECUTIVE" => [522.0, 756.0], // = (  184 x 267  ) mm  = (  7.25 x 10.50 ) in
        "ORGANIZERL" => [396.0, 612.0], // = (  140 x 216  ) mm  = (  5.50 x 8.50  ) in
        "STATEMENT" => [396.0, 612.0], // = (  140 x 216  ) mm  = (  5.50 x 8.50  ) in
        "MEMO" => [396.0, 612.0], // = (  140 x 216  ) mm  = (  5.50 x 8.50  ) in
        "FOOLSCAP" => [595.44, 936.0], // = (  210 x 330  ) mm  = (  8.27 x 13.00 ) in
        "COMPACT" => [306.0, 486.0], // = (  108 x 171  ) mm  = (  4.25 x 6.75  ) in
        "ORGANIZERJ" => [198.0, 360.0], // = (   70 x 127  ) mm  = (  2.75 x 5.00  ) in
        ...
        "P5" => [396.85, 609.449], // = (  140 x 215  ) mm  = (  5.51 x 8.46  ) in
        "P6" => [303.307, 396.85], // = (  107 x 140  ) mm  = (  4.21 x 5.51  ) in
        // North American Architectural Sizes
        "ARCH_E" => [2592.0, 3456.0], // = (  914 x 1219 ) mm  = ( 36.00 x 48.00 ) in
        "ARCH_E1" => [2160.0, 3024.0], // = (  762 x 1067 ) mm  = ( 30.00 x 42.00 ) in
        "ARCH_D" => [1728.0, 2592.0], // = (  610 x 914  ) mm  = ( 24.00 x 36.00 ) in
        "BROADSHEET" => [1296.0, 1728.0], // = (  457 x 610  ) mm  = ( 18.00 x 24.00 ) in
        "ARCH_C" => [1296.0, 1728.0], // = (  457 x 610  ) mm  = ( 18.00 x 24.00 ) in
        "ARCH_B" => [864.0, 1296.0], // = (  305 x 457  ) mm  = ( 12.00 x 18.00 ) in
        "ARCH_A" => [648.0, 864.0], // = (  229 x 305  ) mm  = (  9.00 x 12.00 ) in
        // -- North American Envelope Sizes
        // - Announcement Envelopes
        "ANNENV_A2" => [314.64, 414.0], // = (  111 x 146  ) mm  = (  4.37 x 5.75  ) in
        "ANNENV_A6" => [342.0, 468.0], // = (  121 x 165  ) mm  = (  4.75 x 6.50  ) in
        "ANNENV_A7" => [378.0, 522.0], // = (  133 x 184  ) mm  = (  5.25 x 7.25  ) in
        "ANNENV_A8" => [396.0, 584.64], // = (  140 x 206  ) mm  = (  5.50 x 8.12  ) in
        "ANNENV_A10" => [450.0, 692.64], // = (  159 x 244  ) mm  = (  6.25 x 9.62  ) in
        "ANNENV_SLIM" => [278.64, 638.64], // = (   98 x 225  ) mm  = (  3.87 x 8.87  ) in
        // - Commercial Envelopes
        "COMMENV_N6_1/4" => [252.0, 432.0], // = (   89 x 152  ) mm  = (  3.50 x 6.00  ) in
        "COMMENV_N6_3/4" => [260.64, 468.0], // = (   92 x 165  ) mm  = (  3.62 x 6.50  ) in
        "COMMENV_N8" => [278.64, 540.0], // = (   98 x 191  ) mm  = (  3.87 x 7.50  ) in
        "COMMENV_N9" => [278.64, 638.64], // = (   98 x 225  ) mm  = (  3.87 x 8.87  ) in
        "COMMENV_N10" => [296.64, 684.0], // = (  105 x 241  ) mm  = (  4.12 x 9.50  ) in
        "COMMENV_N11" => [324.0, 746.64], // = (  114 x 263  ) mm  = (  4.50 x 10.37 ) in
        "COMMENV_N12" => [342.0, 792.0], // = (  121 x 279  ) mm  = (  4.75 x 11.00 ) in
        "COMMENV_N14" => [360.0, 828.0], // = (  127 x 292  ) mm  = (  5.00 x 11.50 ) in
        // - Catalogue Envelopes
        "CATENV_N1" => [432.0, 648.0], // = (  152 x 229  ) mm  = (  6.00 x 9.00  ) in
        "CATENV_N1_3/4" => [468.0, 684.0], // = (  165 x 241  ) mm  = (  6.50 x 9.50  ) in
        "CATENV_N2" => [468.0, 720.0], // = (  165 x 254  ) mm  = (  6.50 x 10.00 ) in
        "CATENV_N3" => [504.0, 720.0], // = (  178 x 254  ) mm  = (  7.00 x 10.00 ) in
        "CATENV_N6" => [540.0, 756.0], // = (  191 x 267  ) mm  = (  7.50 x 10.50 ) in
        "CATENV_N7" => [576.0, 792.0], // = (  203 x 279  ) mm  = (  8.00 x 11.00 ) in
        "CATENV_N8" => [594.0, 810.0], // = (  210 x 286  ) mm  = (  8.25 x 11.25 ) in
        "CATENV_N9_1/2" => [612.0, 756.0], // = (  216 x 267  ) mm  = (  8.50 x 10.50 ) in
        "CATENV_N9_3/4" => [630.0, 810.0], // = (  222 x 286  ) mm  = (  8.75 x 11.25 ) in
        "CATENV_N10_1/2" => [648.0, 864.0], // = (  229 x 305  ) mm  = (  9.00 x 12.00 ) in
        "CATENV_N12_1/2" => [684.0, 900.0], // = (  241 x 318  ) mm  = (  9.50 x 12.50 ) in
        "CATENV_N13_1/2" => [720.0, 936.0], // = (  254 x 330  ) mm  = ( 10.00 x 13.00 ) in
        "CATENV_N14_1/4" => [810.0, 882.0], // = (  286 x 311  ) mm  = ( 11.25 x 12.25 ) in
        "CATENV_N14_1/2" => [828.0, 1044.0], // = (  292 x 368  ) mm  = ( 11.50 x 14.50 ) in
        ...
        "PA5" => [396.85, 595.276], // = (  140 x 210  ) mm  = (  5.51 x 8.27  ) in
        "PA6" => [297.638, 396.85], // = (  105 x 140  ) mm  = (  4.13 x 5.51  ) in
        ...
        "L" => [252.283, 360.0], // = (   89 x 127  ) mm  = (  3.50 x 5.00  ) in
        "3R" => [252.283, 360.0], // = (   89 x 127  ) mm  = (  3.50 x 5.00  ) in
        ...
        "2L" => [360.0, 504.567], // = (  127 x 178  ) mm  = (  5.00 x 7.01  ) in
        "5R" => [360.0, 504.567], // = (  127 x 178  ) mm  = (  5.00 x 7.01  ) in
        "8P" => [430.866, 575.433], // = (  152 x 203  ) mm  = (  5.98 x 7.99  ) in
        "6R" => [430.866, 575.433], // = (  152 x 203  ) mm  = (  5.98 x 7.99  ) in
        "6P" => [575.433, 720.0], // = (  203 x 254  ) mm  = (  7.99 x 10.00 ) in
        "8R" => [575.433, 720.0], // = (  203 x 254  ) mm  = (  7.99 x 10.00 ) in
        "6PW" => [575.433, 864.567], // = (  203 x 305  ) mm  = (  7.99 x 12.01 ) in
        "S8R" => [575.433, 864.567], // = (  203 x 305  ) mm  = (  7.99 x 12.01 ) in
        "4P" => [720.0, 864.567], // = (  254 x 305  ) mm  = ( 10.00 x 12.01 ) in
        "10R" => [720.0, 864.567], // = (  254 x 305  ) mm  = ( 10.00 x 12.01 ) in
        "4PW" => [720.0, 1080.0], // = (  254 x 381  ) mm  = ( 10.00 x 15.00 ) in
        "S10R" => [720.0, 1080.0], // = (  254 x 381  ) mm  = ( 10.00 x 15.00 ) in
        "11R" => [790.866, 1009.134], // = (  279 x 356  ) mm  = ( 10.98 x 14.02 ) in
        "S11R" => [790.866, 1224.567], // = (  279 x 432  ) mm  = ( 10.98 x 17.01 ) in
        "12R" => [864.567, 1080.0], // = (  305 x 381  ) mm  = ( 12.01 x 15.00 ) in
        ...
        // Billboards
        "4SHEET" => [2880.0, 4320.0], // = ( 1016 x 1524 ) mm  = ( 40.00 x 60.00 ) in
        "6SHEET" => [3401.575, 5102.362], // = ( 1200 x 1800 ) mm  = ( 47.24 x 70.87 ) in
        "12SHEET" => [8640.0, 4320.0], // = ( 3048 x 1524 ) mm  = (120.00 x 60.00 ) in
        "16SHEET" => [5760.0, 8640.0], // = ( 2032 x 3048 ) mm  = ( 80.00 x 120.00) in
        "32SHEET" => [11520.0, 8640.0], // = ( 4064 x 3048 ) mm  = (160.00 x 120.00) in
        "48SHEET" => [17280.0, 8640.0], // = ( 6096 x 3048 ) mm  = (240.00 x 120.00) in
        "64SHEET" => [23040.0, 8640.0], // = ( 8128 x 3048 ) mm  = (320.00 x 120.00) in
        "96SHEET" => [34560.0, 8640.0], // = (12192 x 3048 ) mm  = (480.00 x 120.00) in
        // -- Old European Sizes
        // - Old Imperial English Sizes
        "EN_EMPEROR" => [3456.0, 5184.0], // = ( 1219 x 1829 ) mm  = ( 48.00 x 72.00 ) in
        "EN_ANTIQUARIAN" => [2232.0, 3816.0], // = (  787 x 1346 ) mm  = ( 31.00 x 53.00 ) in
        "EN_GRAND_EAGLE" => [2070.0, 3024.0], // = (  730 x 1067 ) mm  = ( 28.75 x 42.00 ) in
        "EN_DOUBLE_ELEPHANT" => [1926.0, 2880.0], // = (  679 x 1016 ) mm  = ( 26.75 x 40.00 ) in
        "EN_ATLAS" => [1872.0, 2448.0], // = (  660 x 864  ) mm  = ( 26.00 x 34.00 ) in
        "EN_COLOMBIER" => [1692.0, 2484.0], // = (  597 x 876  ) mm  = ( 23.50 x 34.50 ) in
        "EN_ELEPHANT" => [1656.0, 2016.0], // = (  584 x 711  ) mm  = ( 23.00 x 28.00 ) in
        "EN_DOUBLE_DEMY" => [1620.0, 2556.0], // = (  572 x 902  ) mm  = ( 22.50 x 35.50 ) in
        "EN_IMPERIAL" => [1584.0, 2160.0], // = (  559 x 762  ) mm  = ( 22.00 x 30.00 ) in
        "EN_PRINCESS" => [1548.0, 2016.0], // = (  546 x 711  ) mm  = ( 21.50 x 28.00 ) in
        "EN_CARTRIDGE" => [1512.0, 1872.0], // = (  533 x 660  ) mm  = ( 21.00 x 26.00 ) in
        "EN_DOUBLE_LARGE_POST" => [1512.0, 2376.0], // = (  533 x 838  ) mm  = ( 21.00 x 33.00 ) in
        "EN_ROYAL" => [1440.0, 1800.0], // = (  508 x 635  ) mm  = ( 20.00 x 25.00 ) in
        "EN_SHEET" => [1404.0, 1692.0], // = (  495 x 597  ) mm  = ( 19.50 x 23.50 ) in
        "EN_HALF_POST" => [1404.0, 1692.0], // = (  495 x 597  ) mm  = ( 19.50 x 23.50 ) in
        "EN_SUPER_ROYAL" => [1368.0, 1944.0], // = (  483 x 686  ) mm  = ( 19.00 x 27.00 ) in
        "EN_DOUBLE_POST" => [1368.0, 2196.0], // = (  483 x 775  ) mm  = ( 19.00 x 30.50 ) in
        "EN_MEDIUM" => [1260.0, 1656.0], // = (  445 x 584  ) mm  = ( 17.50 x 23.00 ) in
        "EN_DEMY" => [1260.0, 1620.0], // = (  445 x 572  ) mm  = ( 17.50 x 22.50 ) in
        "EN_LARGE_POST" => [1188.0, 1512.0], // = (  419 x 533  ) mm  = ( 16.50 x 21.00 ) in
        "EN_COPY_DRAUGHT" => [1152.0, 1440.0], // = (  406 x 508  ) mm  = ( 16.00 x 20.00 ) in
        "EN_POST" => [1116.0, 1386.0], // = (  394 x 489  ) mm  = ( 15.50 x 19.25 ) in
        "EN_CROWN" => [1080.0, 1440.0], // = (  381 x 508  ) mm  = ( 15.00 x 20.00 ) in
        "EN_PINCHED_POST" => [1062.0, 1332.0], // = (  375 x 470  ) mm  = ( 14.75 x 18.50 ) in
        "EN_BRIEF" => [972.0, 1152.0], // = (  343 x 406  ) mm  = ( 13.50 x 16.00 ) in
        "EN_FOOLSCAP" => [972.0, 1224.0], // = (  343 x 432  ) mm  = ( 13.50 x 17.00 ) in
        "EN_SMALL_FOOLSCAP" => [954.0, 1188.0], // = (  337 x 419  ) mm  = ( 13.25 x 16.50 ) in
        "EN_POTT" => [900.0, 1080.0], // = (  318 x 381  ) mm  = ( 12.50 x 15.00 ) in
        // - Old Imperial Belgian Sizes
        "BE_GRAND_AIGLE" => [1984.252, 2948.031], // = (  700 x 1040 ) mm  = ( 27.56 x 40.94 ) in
        "BE_COLOMBIER" => [1757.48, 2409.449], // = (  620 x 850  ) mm  = ( 24.41 x 33.46 ) in
        "BE_DOUBLE_CARRE" => [1757.48, 2607.874], // = (  620 x 920  ) mm  = ( 24.41 x 36.22 ) in
        ...
        "BE_RAISIN" => [1417.323, 1842.52], // = (  500 x 650  ) mm  = ( 19.69 x 25.59 ) in
        ...
        "FR_DOUBLE_RAISIN" => [1842.52, 2834.646], // = (  650 x 1000 ) mm  = ( 25.59 x 39.37 ) in
        "FR_JOURNAL" => [1842.52, 2664.567], // = (  650 x 940  ) mm  = ( 25.59 x 37.01 ) in
        "FR_COLOMBIER_AFFICHE" => [1785.827, 2551.181], // = (  630 x 900  ) mm  = ( 24.80 x 35.43 ) in
        "FR_DOUBLE_CAVALIER" => [1757.48, 2607.874], // = (  620 x 920  ) mm  = ( 24.41 x 36.22 ) in
        ...
        "FR_RAISIN" => [1417.323, 1842.52], // = (  500 x 650  ) mm  = ( 19.69 x 25.59 ) in
        "FR_CAVALIER" => [1303.937, 1757.48], // = (  460 x 620  ) mm  = ( 18.11 x 24.41 ) in
        ...
        "FR_DOUBLE_POT" => [1133.858, 1757.48], // = (  400 x 620  ) mm  = ( 15.75 x 24.41 ) in
        "FR_ECU" => [1133.858, 1474.016], // = (  400 x 520  ) mm  = ( 15.75 x 20.47 ) in
        "FR_COURONNE" => [1020.472, 1303.937], // = (  360 x 460  ) mm  = ( 14.17 x 18.11 ) in
        "FR_TELLIERE" => [963.78, 1247.244], // = (  340 x 440  ) mm  = ( 13.39 x 17.32 ) in
        "FR_POT" => [878.74, 1133.858], // = (  310 x 400  ) mm  = ( 12.20 x 15.75 ) in
    ];
