...
class PDF417
{
    ...
    public function __construct($code, $ecl = -1, $aspectratio = 2, $macro = [])
    {
        ...
        if (!empty($macro)) {
            ...
            $macrocw[] = 928;
            // segment index
            $cw = $this->getCompaction(
                902,
                sprintf("%05d", $macro["segment_index"]),
                false
            );
        ...
        $nce = $numcw + $errsize + 1;
        $cols = round(
            (sqrt(4761 + 68 * $aspectratio * ROWHEIGHT * $nce) - 69) / 34
        );
        // adjust cols
        if ($cols < 1) {
            $cols = 1;
        ...
        if ($size > 928) {
            ...
            if (
                abs($aspectratio - (17 * 29) / 32) <
                abs($aspectratio - (17 * 16) / 58)
            ) {
                $cols = 29;
                $rows = 32;
            } else {
                ...
                foreach ($textseq[1] as $txtseq) {
                    ...
                    if ($txtseq[1] > 0) {
                        ...
                        if (strlen($prevtxtseq) > 0) {
                            // add BYTE sequence
                            if (
                                strlen($prevtxtseq) == 1 and
                                (count($sequence_array) > 0 and
                                    $sequence_array[
                                        count($sequence_array) - 1
                                    ][0] ==
                                        900)
                            ) {
