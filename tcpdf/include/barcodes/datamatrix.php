...
class Datamatrix
{
        ...
        if ($params[11] < $nd) {
            ...
            if ($params[11] - $nd > 1 and $cw[$nd - 1] != 254) {
                if ($this->last_enc == ENC_EDF) {
                    ...
                    $this->last_enc != ENC_ASCII and
                    $this->last_enc != ENC_BASE256
                ) {
                    // switch to ASCII encoding
                    $cw[] = 254;
                    ++$nd;
    ...
    protected function isCharMode($chr, $mode)
    {
        ...
        switch ($mode) {
            ...
            case ENC_BASE256:
                ...
                $status =
                    ($chr == 232 or $chr == 233 or $chr == 234 or $chr == 241);
                break;
            case ENC_ASCII_EXT:
    ...
    protected function lookAheadTest($data, $pos, $mode)
    {
        ...
        while (true) {
            ...
            // STEP O
            if (
                $this->isCharMode($chr, ENC_X12) or
                $this->isCharMode($chr, ENC_C40)
            ) {
                $numch[ENC_X12] += 2 / 3;
            } elseif ($this->isCharMode($chr, ENC_ASCII_EXT)) {
                $numch[ENC_X12] += 13 / 3;
            ...
            if ($charscount >= 4) {
                ...
                if (
                    $numch[ENC_BASE256] + 1 <= $numch[ENC_ASCII] or
                    $numch[ENC_BASE256] + 1 <
                        min(
                            $numch[ENC_C40],
                            $numch[ENC_TXT],
                    ...
                    if ($numch[ENC_C40] == $numch[ENC_X12]) {
                        ...
                        while ($k < $data_length) {
                            ...
                            if ($this->isCharMode($tmpchr, ENC_X12)) {
                                ...
                                !(
                                    $this->isCharMode($tmpchr, ENC_X12) or
                                    $this->isCharMode($tmpchr, ENC_C40)
                                )
                            ) {
                                break;
                            }
        ...
        while ($pos < $data_length) {
            ...
            switch ($enc) {
                case ENC_ASCII:
                    ...
                    if (
                        $data_length > 1 and
                        $pos < $data_length - 1 and
                        ($this->isCharMode(ord($data[$pos]), ENC_ASCII_NUM) and
                            $this->isCharMode(
                                ord($data[$pos + 1]),
                                ENC_ASCII_NUM
                            ))
                    ...
                    do {
                        ...
                        if (isset($charset[$chr])) {
                            ...
                            if (isset($this->chset["SH1"][$chr])) {
                                ...
                                $enc == ENC_C40 and
                                isset($this->chset["S3C"][$chr])
                            ) {
                                $temp_cw[] = 2; // shift 3
                                $shiftset = $this->chset["S3C"];
                            } elseif (
                                $enc == ENC_TXT and
                                isset($this->chset["S3T"][$chr])
                            ) {
                                $temp_cw[] = 2; // shift 3
                        ...
                        if ($p >= 3) {
                            ...
                            if ($newenc != $enc) {
                                ...
                                if ($enc != ENC_ASCII) {
                                    ...
                                    $cw[] = $this->getSwitchEncodingCodeword(
                                        ENC_ASCII
                                    );
                                    ++$cw_num;
                                }
