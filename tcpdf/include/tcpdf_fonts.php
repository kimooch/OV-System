...
class TCPDF_FONTS
{
     ...
     * @public static
     */
    public static function addTTFfont(
        $fontfile,
        $fonttype = "",
        $enc = "",
        $flags = 32,
        ...
        if ($fmetric["type"] == "TrueType" or $fmetric["type"] == "Type1") {
            if (
                !empty($enc) and
                $enc != "cp1252" and
                isset(TCPDF_FONT_DATA::$encmap[$enc])
            ) {
        ...
        if ($fmetric["type"] == "Type1") {
            ...
            if (
                preg_match("#/ForceBold[\s]*([^\s]*)#", $eplain, $matches) > 0
            ) {
                if ($matches[1] == "true") {
            ...
            if (
                preg_match("#/BlueValues[\s]*\[([^\]]*)#", $eplain, $matches) >
                0
            ) {
            ...
            foreach ($matches as $k => $v) {
                ...
                while ($i < $clen) {
                    if ($ccom[$i] < 32) {
                        ...
                        $cdec[$ck] =
                            ($ccom[$i] - 247) * 256 + $ccom[$i + 1] + 108;
                        $i += 2;
                    } elseif ($ccom[$i] >= 251 and $ccom[$i] <= 254) {
                        $cdec[$ck] =
                            -($ccom[$i] - 251) * 256 - $ccom[$i + 1] - 108;
                        $i += 2;
                    } elseif ($ccom[$i] == 255) {
                        $sval =
                            chr($ccom[$i + 1]) .
            ...
            for ($cid = 0; $cid <= 255; ++$cid) {
                if (isset($cwidths[$cid])) {
                    ...
                    $fmetric["cw"] .=
                        "," . $cid . "=>" . $fmetric["MissingWidth"];
                }
            }
            $fmetric["AvgWidth"] = round(
                $fmetric["AvgWidth"] / count($cwidths)
            );
        } else {
            // ---------- TRUE TYPE ----------
            ...
            for ($i = 0; $i < $numTables; ++$i) {
                ...
                $offset += 4;
                $table[$tag] = [];
                $table[$tag]["checkSum"] = TCPDF_STATIC::_getULONG(
                    $font,
                    $offset
                );
                $offset += 4;
                $table[$tag]["offset"] = TCPDF_STATIC::_getULONG(
                    $font,
                    $offset
                );
                $offset += 4;
                $table[$tag]["length"] = TCPDF_STATIC::_getULONG(
                    $font,
                    $offset
                );
                $offset += 4;
            ...
            if ($short_offset) {
                ...
                for ($i = 0; $i < $tot_num_glyphs; ++$i) {
                    $indexToLoc[$i] =
                        TCPDF_STATIC::_getUSHORT($font, $offset) * 2;
                    if (
                        isset($indexToLoc[$i - 1]) &&
                        $indexToLoc[$i] == $indexToLoc[$i - 1]
                    ) {
            ...
            for ($i = 0; $i < $numEncodingTables; ++$i) {
                $encodingTables[$i]["platformID"] = TCPDF_STATIC::_getUSHORT(
                    $font,
                    $offset
                );
                $offset += 2;
                $encodingTables[$i]["encodingID"] = TCPDF_STATIC::_getUSHORT(
                    $font,
                    $offset
                );
                $offset += 2;
                $encodingTables[$i]["offset"] = TCPDF_STATIC::_getULONG(
                    $font,
                    $offset
                );
                $offset += 4;
            ...
            // xAvgCharWidth
            $fmetric["AvgWidth"] = round(
                TCPDF_STATIC::_getFWORD($font, $offset) * $urk
            );
            $offset += 2;
            // usWeightClass
            $usWeightClass = round(
                TCPDF_STATIC::_getUFWORD($font, $offset) * $urk
            );
            ...
            $offset += 2;
            $isFixedPitch =
                TCPDF_STATIC::_getULONG($font, $offset) == 0 ? false : true;
            $offset += 2;
            if ($isFixedPitch) {
                $fmetric["Flags"] |= 1;
            }
            ...
            foreach ($encodingTables as $enctable) {
                ...
                if (
                    $enctable["platformID"] == $platid and
                    $enctable["encodingID"] == $encid
                ) {
                    ...
                    switch ($format) {
                        ...
                        case 2:
                            ...
                            for ($i = 0; $i < 256; ++$i) {
                                ...
                                $subHeaderKeys[$i] =
                                    TCPDF_STATIC::_getUSHORT($font, $offset) /
                                    8;
                                $offset += 2;
                        ...
                        case 4:
                            ...
                            for ($k = 0; $k < $segCount; ++$k) {
                                $endCount[$k] = TCPDF_STATIC::_getUSHORT(
                                    $font,
                                    $offset
                                );
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $segCount; ++$k) {
                                $startCount[$k] = TCPDF_STATIC::_getUSHORT(
                                    $font,
                                    $offset
                                );
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $segCount; ++$k) {
                                $idDelta[$k] = TCPDF_STATIC::_getUSHORT(
                                    $font,
                                    $offset
                                );
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $segCount; ++$k) {
                                $idRangeOffset[$k] = TCPDF_STATIC::_getUSHORT(
                                    $font,
                                    $offset
                                );
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $gidlen; ++$k) {
                                $glyphIdArray[$k] = TCPDF_STATIC::_getUSHORT(
                                    $font,
                                    $offset
                                );
                                $offset += 2;
                        ...
                        case 6:
                            ...
                            $offset += 4; // skip length and version/language
                            $firstCode = TCPDF_STATIC::_getUSHORT(
                                $font,
                                $offset
                            );
                            $offset += 2;
                            $entryCount = TCPDF_STATIC::_getUSHORT(
                                $font,
                                $offset
                            );
                            $offset += 2;
                        ...
                        case 8:
                            ...
                            $offset += 4;
                            for ($i = 0; $i < $nGroups; ++$i) {
                                $startCharCode = TCPDF_STATIC::_getULONG(
                                    $font,
                                    $offset
                                );
                                $offset += 4;
                                $endCharCode = TCPDF_STATIC::_getULONG(
                                    $font,
                                    $offset
                                );
                                $offset += 4;
                                $startGlyphID = TCPDF_STATIC::_getULONG(
                                    $font,
                                    $offset
                                );
                                $offset += 4;
                        ...
                        case 12:
                            ...
                            for ($k = 0; $k < $nGroups; ++$k) {
                                ...
                                $offset += 4;
                                $startGlyphCode = TCPDF_STATIC::_getULONG(
                                    $font,
                                    $offset
                                );
                                $offset += 4;
            ...
            for ($cid = 0; $cid <= 65535; ++$cid) {
                if (isset($ctg[$cid])) {
                    ...
                    if ($addcbbox and isset($indexToLoc[$ctg[$cid]])) {
                        $offset =
                            $table["glyf"]["offset"] + $indexToLoc[$ctg[$cid]];
                        $xMin = round(
                            TCPDF_STATIC::_getFWORD($font, $offset + 2) * $urk
                        );
                        $yMin = round(
                            TCPDF_STATIC::_getFWORD($font, $offset + 4) * $urk
                        );
                        $xMax = round(
                            TCPDF_STATIC::_getFWORD($font, $offset + 6) * $urk
                        );
                        $yMax = round(
                            TCPDF_STATIC::_getFWORD($font, $offset + 8) * $urk
                        );
                        $fmetric["cbbox"] .=
                            "," .
                            $cid .
        ...
        if ($fmetric["type"] == "Type1") {
            ...
            if ($fmetric["type"] == "cidfont0") {
                ...
                foreach ($ctg as $cid => $gid) {
                    $cidtogidmap = self::updateCIDtoGIDmap(
                        $cidtogidmap,
                        $cid,
                        $ctg[$cid]
                    );
        ...
        if (!empty($fmetric["cbbox"])) {
            $pfile .=
                '$cbbox=array(' . substr($fmetric["cbbox"], 1) . ");" . "\n";
        }
    ...
    public static function _getTrueTypeFontSubset($font, $subsetchars)
    {
            ...
            switch ($format) {
                ...
                case 2:
                    ...
                    for ($i = 0; $i < 256; ++$i) {
                        // Array that maps high bytes to subHeaders: value is subHeader index * 8.
                        $subHeaderKeys[$i] =
                            TCPDF_STATIC::_getUSHORT($font, $offset) / 8;
                        $offset += 2;
                        if ($numSubHeaders < $subHeaderKeys[$i]) {
                            $numSubHeaders = $subHeaderKeys[$i];
                        }
                    ...
                    $numGlyphIndexArray = 0;
                    for ($k = 0; $k < $numSubHeaders; ++$k) {
                        $subHeaders[$k]["firstCode"] = TCPDF_STATIC::_getUSHORT(
                            $font,
                            $offset
                        );
                        $offset += 2;
                        ...
                        $offset += 2;
                        $subHeaders[$k]["idDelta"] = TCPDF_STATIC::_getUSHORT(
                            $font,
                            $offset
                        );
                        $offset += 2;
                    ...
                    for ($i = 0; $i < 256; ++$i) {
                        ...
                        if ($k == 0) {
                            ...
                            $start_byte = $subHeaders[$k]["firstCode"];
                            $end_byte =
                                $start_byte + $subHeaders[$k]["entryCount"];
                ...
                if (isset($indexToLoc[$key])) {
                    ...
                    if ($numberOfContours < 0) {
                        ...
                        do {
                            ...
                            $offset += 2;
                            $glyphIndex = TCPDF_STATIC::_getUSHORT(
                                $font,
                                $offset
                            );
                            $offset += 2;
        ...
        $table["loca"]["checkSum"] = self::_getTTFtableChecksum(
            $table["loca"]["data"],
            $table["loca"]["length"]
        );
        ...
        $table["glyf"]["checkSum"] = self::_getTTFtableChecksum(
            $table["glyf"]["data"],
            $table["glyf"]["length"]
        );
        // rebuild font
        ...
        $font =
            substr($font, 0, $table["head"]["offset"] + 8) .
    ...
    {
        if (
            !defined("K_PATH_FONTS") and
            is_dir($fdir = realpath(dirname(__FILE__) . "/../fonts"))
        ) {
            if (substr($fdir, -1) != "/") {
                $fdir .= "/";
            }
    ...
    public static function getFontFullPath($file, $fontdir = false)
    {
        ...
        if (
            $fontdir !== false and
            @TCPDF_STATIC::file_exists($fontdir . $file)
        ) {
     ...
     * @public static
     */
    public static function UTF8ArrSubString(
        $strarr,
        $start = "",
        $end = "",
        $unicode = true
    ...
    public static function UTF8ArrToLatin1Arr($unicode)
    {
        ...
        foreach ($unicode as $char) {
            if ($char < 256) {
                ...
                array_key_exists($char, TCPDF_FONT_DATA::$uni_utf8tolatin)
            ) {
                // map from UTF-8
    ...
    public static function getUniord($uch)
    {
        if (function_exists("mb_convert_encoding")) {
            list(, $char) = @unpack(
                "N",
                mb_convert_encoding($uch, "UCS-4BE", "UTF-8")
            );
            if ($char >= 0) {
                return $char;
            }
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if ($countbytes == 0) {
                ...
                if ($countbytes == $numbytes) {
                    ...
                    if (
                        $char >= 0xd800 and $char <= 0xdfff or
                        $char >= 0x10ffff
                    ) {
    ...
    public static function UTF8StringToArray($str, $isunicode, &$currentfont)
    {
        ...
        if (is_array($currentfont["subsetchars"]) && is_array($carr)) {
            ...
            $currentfont["subsetchars"] = array_merge(
     ...
     * @public static
     */
    public static function UTF8ToUTF16BE(
        $str,
        $setbom,
        $isunicode,
        &$currentfont
     ...
     * @public static
     */
    public static function utf8StrRev(
        $str,
        $setbom,
        $forcertl,
        $isunicode,
        ...
        return self::utf8StrArrRev(
            self::UTF8StringToArray($str, $isunicode, $currentfont),
            $str,
            $setbom,
            $forcertl,
            $isunicode,
     ...
     * @public static
     */
    public static function utf8StrArrRev(
        $arr,
        $str,
        $setbom,
        $forcertl,
     ...
     * @public static
     */
    public static function utf8Bidi(
        $ta,
        $str,
        $forcertl,
        $isunicode,
        ...
        if (
            !(
                $forcertl or
                $arabic or
                preg_match(TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $str)
            )
        ) {
            return $ta;
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (
                ...
                if (
                    $chardata[$i]["type"] == "ES" and
                    $chardata[$i - 1]["type"] == "EN" and
                    $chardata[$i + 1]["type"] == "EN"
                ) {
                    $chardata[$i]["type"] = "EN";
                } elseif (
                    $chardata[$i]["type"] == "CS" and
                    $chardata[$i - 1]["type"] == "EN" and
                    $chardata[$i + 1]["type"] == "EN"
                ) {
                    $chardata[$i]["type"] = "EN";
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (
                ...
                if (
                    ...
                    $chardata[$i - 1]["type"] == "L" and
                    $chardata[$i + 1]["type"] == "L"
                ) {
                    $chardata[$i]["type"] = "L";
                } elseif (
                    $chardata[$i]["type"] == "N" and
                    ($chardata[$i - 1]["type"] == "R" or
                        $chardata[$i - 1]["type"] == "EN" or
                        $chardata[$i - 1]["type"] == "AN") and
                    ($chardata[$i + 1]["type"] == "R" or
                        $chardata[$i + 1]["type"] == "EN" or
                        $chardata[$i + 1]["type"] == "AN")
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]["type"] == "B" or $chardata[$i]["type"] == "S") {
                ...
                while ($j < $numchars) {
                    if (
                        $chardata[$j]["type"] == "B" or
                        $chardata[$j]["type"] == "S" or
        ...
        if ($arabic) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if (
                    TCPDF_FONT_DATA::$uni_type[$chardata[$i]["char"]] == "AL" or
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                ...
                if (TCPDF_FONT_DATA::$uni_type[$thischar["char"]] == "AL") {
                    ...
                    if (
                        $prevchar !== false and
                        $nextchar !== false and
                        (TCPDF_FONT_DATA::$uni_type[$prevchar["char"]] ==
                            "AL" or
                            TCPDF_FONT_DATA::$uni_type[$prevchar["char"]] ==
                                "NSM") and
                        (TCPDF_FONT_DATA::$uni_type[$nextchar["char"]] ==
                            "AL" or
                            TCPDF_FONT_DATA::$uni_type[$nextchar["char"]] ==
                                "NSM") and
                        ...
                        if (in_array($prevchar["char"], $endedletter)) {
                            if (isset($arabicarr[$thischar["char"]][2])) {
                                // initial
                                $chardata2[$i]["char"] =
                                    $arabicarr[$thischar["char"]][2];
                            }
                        } else {
                            if (isset($arabicarr[$thischar["char"]][3])) {
                                // medial
                                $chardata2[$i]["char"] =
                                    $arabicarr[$thischar["char"]][3];
                            }
                        }
                    } elseif (
                        $nextchar !== false and
                        ...
                        if (
                            ...
                            if (
                                $prevchar !== false and
                                in_array($prevchar["char"], $endedletter)
                            ) {
                                if (isset($arabicarr[$thischar["char"]][0])) {
                                    // isolated
                                    $chardata2[$i]["char"] =
                                        $arabicarr[$thischar["char"]][0];
                                }
                            } else {
                                if (isset($arabicarr[$thischar["char"]][1])) {
                                    // final
                                    $chardata2[$i]["char"] =
                                        $arabicarr[$thischar["char"]][1];
                                }
                            }
                        }
            ...
            for ($i = 0; $i < $numchars - 1; ++$i) {
                if (
                    $chardata2[$i]["char"] == 1617 and
                    isset(
                        TCPDF_FONT_DATA::$uni_diacritics[
                            $chardata2[$i + 1]["char"]
                        ]
                    )
                ) {
        ...
        for ($j = $maxlevel; $j > 0; $j--) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($chardata[$i]["level"] >= $j) {
                    ...
                    if (
                        isset(
                            TCPDF_FONT_DATA::$uni_mirror[$chardata[$i]["char"]]
                        )
                    ) {
