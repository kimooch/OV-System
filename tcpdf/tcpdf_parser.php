...
class TCPDF_PARSER
{
    ...
    protected function setConfig($cfg)
    {
        ...
        if (isset($cfg["ignore_filter_decoding_errors"])) {
            $this->cfg["ignore_filter_decoding_errors"] = !!$cfg[
                "ignore_filter_decoding_errors"
            ];
        }
        if (isset($cfg["ignore_missing_filter_decoders"])) {
            $this->cfg["ignore_missing_filter_decoders"] = !!$cfg[
    ...
    protected function decodeXref($startxref, $xref = [])
    {
        ...
        $offset =
            $startxref +
            strspn($this->pdfdata, "\x00\x09\x0a\x0c\x0d\x20", $startxref);
        ...
        if (
            ...
            if (!isset($xref["trailer"]) or empty($xref["trailer"])) {
                ...
                if (
                    preg_match(
                        "/Root[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i",
                        $trailer_data,
                        $matches
                    ) > 0
                ) {
                    $xref["trailer"]["root"] =
                        intval($matches[1]) . "_" . intval($matches[2]);
                }
                if (
                    preg_match(
                        "/Encrypt[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i",
                        $trailer_data,
                        $matches
                    ) > 0
                ) {
    ...
    protected function decodeXrefStream($startxref, $xref = [])
    {
        ...
        foreach ($sarr as $k => $v) {
            if (
                ...
                $v[0] == "/" and
                $v[1] == "Prev" and
                (isset($sarr[$k + 1]) and $sarr[$k + 1][0] == "numeric")
            ) {
                ...
                if (
                    ...
                    $v[0] == "/" and
                    $v[1] == "Root" and
                    (isset($sarr[$k + 1]) and $sarr[$k + 1][0] == "objref")
                ) {
    ...
    protected function getRawObject($offset = 0)
    {
        ...
        switch ($char) {
            ...
            case ">":
                ...
                if (
                    isset($this->pdfdata[$offset + 1]) and
                    $this->pdfdata[$offset + 1] == $char
                ) {
                    // dictionary object
                    $objtype = $char . $char;
                    $offset += 2;
                    ...
                    if (
                        ...
                        ($endpos = strpos($this->pdfdata, ">", $offset)) !==
                        false
                    ) {
                        $offset = $endpos + 1;
                    }
    ...
    protected function getIndirectObject(
        ...
        do {
            ...
            if (
                $decoding and
                $element[0] == "stream" and
                isset($objdata[$i - 1][0]) and
                $objdata[$i - 1][0] == "<<"
            ) {
                $element[3] = $this->decodeStream(
                    $objdata[$i - 1][1],
                    $element[1]
                );
            }
    ...
    protected function getObjectVal($obj)
    {
        if ($obj[0] == "objref") {
            ...
            if (isset($this->objects[$obj[1]])) {
                ...
                // parse new object
                $this->objects[$obj[1]] = $this->getIndirectObject(
                    $obj[1],
                    $this->xref[$obj[1]],
                    false
                );
        ...
        foreach ($filters as $filter) {
            if (in_array($filter, TCPDF_FILTERS::getAvailableFilters())) {
                try {
                    ...
                    if (
                        $emsg[0] == "~" and
                            !$this->cfg["ignore_missing_filter_decoders"] or
                        $emsg[0] != "~" and
                            !$this->cfg["ignore_filter_decoding_errors"]
                    ) {
