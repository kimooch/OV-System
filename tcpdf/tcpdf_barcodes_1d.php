...
class TCPDFBarcode
{
    ...
    public function getBarcodeSVG($w = 2, $h = 30, $color = "black")
    {
        ...
        header(
            'Content-Disposition: inline; filename="' . md5($code) . '.svg";'
        );
    ...
    public function getBarcodeSVGcode($w = 2, $h = 30, $color = "black")
    {
        ...
        $svg .=
            '<svg width="' .
            round($this->barcode_array["maxw"] * $w, 3) .
            '" height="' .
            $h .
        ...
        $svg .=
            "\t" .
            "<desc>" .
            strtr($this->barcode_array["code"], $repstr) .
            "</desc>" .
            "\n";
        $svg .=
            "\t" . '<g id="bars" fill="' . $color . '" stroke="none">' . "\n";
        // print bars
        $x = 0;
    ...
    {
        $html =
            '<div style="font-size:0;position:relative;width:' .
     ...
     * @protected
     */
    protected function barcode_code39(
        $code,
        $extended = false,
        $checksum = false
    ) {
            ...
            default:
                ...
                if (isset($numseq[1]) and !empty($numseq[1])) {
                    ...
                    if ($end_offset < $len) {
                        $sequence = array_merge(
                            $sequence,
                            $this->get128ABsequence(substr($code, $end_offset))
                        );
                    }
                } else {
        ...
        if ($upce) {
            ...
            if ($tmp == "000" or $tmp == "100" or $tmp == "200") {
                ...
                if ($tmp == "00") {
                    ...
                    $upce_code =
                        substr($code, 2, 3) . substr($code, 10, 2) . "3";
                } else {
                    $tmp = substr($code, 6, 1);
                    if ($tmp == "0") {
                        // manufacturer code ends in 0
                        $upce_code =
                            substr($code, 2, 4) . substr($code, 11, 1) . "4";
                    } else {
